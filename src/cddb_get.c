 ...
 */
char *
get_inet_name (address, char_port, discID, tracks)
	...
	int		discID, tracks;
{
	...
	for (i = 0; i < tracks; i++) 
		{
			...
			counter += cdtoc[i].frame + (75 * cdtoc[i].sec + (60 * cdtoc[i].min));
		}
	...
	if (! (strncmp (answer, "211", 3)))
	...
	else 
		{
			...
			do 
				{
					printf ("Do you want to write information about this album to the server [y/n]? "); 
					fflush (stdout);
			...
			if (toupper (key) == 'Y') 
				{
					close (server_fd);
					filename = get_from_user (discID, tracks, offsets, total_secs, server, address);
					return (filename);
				}
 ...
 */
char *
get_from_user (discID, tracks, offsets, total_secs, server_name, address)
	...
	char	*offsets, *server_name, *address;
{
	...
	for (i = 0; i < tracks; i++) 
		{
			...
			while ((strlen (tmp) == 1) 
					|| ((strstr (tmp, "track")) && (strlen (tmp) < 9) && (strlen (tmp) > 5)));
	...
	snprintf (body, sizeof (body), "# xmcd CD database file\n#\n# Track frame offsets:\n");
	while (token) 
		{
			strcat (body, "#     ");
			strcat (body, token);
	...
	strcat (body, " Copyright (c) 1996-1997 Steve Scherf\n# Submitted via: Lazy ");
 ...
 */
char *
save_to_disk (subdir, cdID, message)
	char	*subdir, *message;
	int		cdID;
{
	FILE	*destination;
	...
	path = (char *) Malloc ((2 + strlen (subdir) + strlen (REAL_PATH)) * sizeof (char));
